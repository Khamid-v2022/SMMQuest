    ...
    function checkNewProviders() {
        ...
        while($row = $result->fetch_assoc()){
            ...
            if($api_check['status'] > 0){
                ...
                $sql_update = "UPDATE providers SET real_url = '" . $real_url . "', is_activated = 1, is_valid_key = " . $api_is_valid . ", is_frozon = " . $is_frozon . ", api_template = '" . $api_check['apiTemplate'] . "', balance = " . $api_check['balance'] . ", currency = '" . ($api_check['currency'] ? $api_check['currency'] : "") . "', updated_at = '" . date('Y-m-d H:i:s') . "' WHERE id = " . $row['id'];
            } else {
                // endpoint is wrong
                $response = urlExists($real_url);
                if(!$response) {
        ...
        while($row = $result->fetch_assoc()){
            ...
            if($row['api_key'] && $row['endpoint'] && $row['api_template']){
                if($row['real_url']){
                    ...
                    if($api_check['status'] == 1) {
                        // valid key
                        $sql = "UPDATE providers SET is_activated = 1, is_valid_key = 1, balance = " . $api_check['balance'] . ", currency = '" . ($api_check['currency'] ? $api_check['currency'] : "") . "', updated_at = '" . date('Y-m-d H:i:s') . "' WHERE id = " . $row['id'];
                        ...
                        if($response){
                            ...
                            if($url == $row['real_url']){
                                ...
                                $sql = "UPDATE providers SET is_activated = 1, is_valid_key = 0, balance = " . $api_check['balance'] . ", currency = '" . ($api_check['currency'] ? $api_check['currency'] : "") . "', updated_at = '" . date('Y-m-d H:i:s') . "' WHERE id = " . $row['id'];
                                $conn->query($sql);
                            } else {
                                ...
                                if($api_check_again['status'] == 1) {
                                    $sql = "UPDATE providers SET real_url = '" . $url . "', is_activated = 1, is_valid_key = 1, balance = " . $api_check_again['balance'] . ", currency = '" . ($api_check_again['currency'] ? $api_check_again['currency'] : "") . "', updated_at = '" . date('Y-m-d H:i:s') . "' WHERE id = " . $row['id'];
                                } else if($api_check_again['status'] == 2){
                                    $sql = "UPDATE providers SET real_url = '" . $url . "', is_activated = 1, is_valid_key = 0, balance = " . $api_check_again['balance'] . ", currency = '" . ($api_check_again['currency'] ? $api_check_again['currency'] : "") . "', updated_at = '" . date('Y-m-d H:i:s') . "' WHERE id = " . $row['id'];
                                }
                    ...
                    if($response){
                        ...
                        if($api_check['status'] == 1) {
                            $sql = "UPDATE providers SET real_url = '" . $url . "', is_activated = 1, is_valid_key = 1, balance = " . $api_check['balance'] . ", currency = '" . ($api_check['currency'] ? $api_check['currency'] : "") . "', updated_at = '" . date('Y-m-d H:i:s') . "' WHERE id = " . $row['id'];
                        } else if($api_check['status'] == 2){
                            // invalid key
                            $sql = "UPDATE providers SET real_url = '" . $url . "', is_activated = 1, is_valid_key = 0, balance = " . $api_check['balance'] . ", currency = '" . ($api_check['currency'] ? $api_check['currency'] : "") . "', updated_at = '" . date('Y-m-d H:i:s') . "' WHERE id = " . $row['id'];
                        } else if($api_check['status'] == 3){
                            // frozon
